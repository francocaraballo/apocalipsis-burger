...
export function ProductCard({ product, onAddedToCart }: ProductCardProps) {
	...
	const cartItemQuantity = items.reduce((acc, i) => {
		if (
			...
			i.product.id.startsWith(`${product.id}-`)
		) {
	...
	const finalPrice =
		product.prices && product.prices[sizeOption.key] !== undefined
		...
		>
			...
			<div className='relative w-full' style={{ height: '200px' }}>
				...
				<div
					...
					style={{
						background:
							'linear-gradient(to top, rgba(32,30,26,0.85) 0%, rgba(32,30,26,0.1) 50%, transparent 100%)',
					}}
					aria-hidden='true'
				/>
						...
						<div className='relative'>
							...
							>
								{SIZE_OPTIONS.map((opt, i) => {
									if (
										!product.prices ||
										product.prices[opt.key] === undefined
									)
										return null;
									return (
